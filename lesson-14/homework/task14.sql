...
select 
    id,
    stuff(vals, 1, 2, substring(vals, 3, 1) + ',' + substring(vals, 1, 1)) as swapped
from multiplevals
...
select 
    ...
    left(@str, patindex('%[0-9]%', @str + 'x') - 1) as characters,
    substring(@str, patindex('%[0-9]%', @str + 'x'), len(@str)) as numbers
